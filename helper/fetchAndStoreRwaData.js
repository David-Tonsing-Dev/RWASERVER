  ...
  while (true) {
    try {
      ...
      for (const coin of coins) {
        ...
        const updated = await CoingeckoToken.findOneAndUpdate(
          { id: coin.id },
          { $set: payload }
        );
      ...
      console.error("Error fetching RWA data:", error.message);
      break;
    }
  }

  fetchedIds.add("condo");

  try {
    const result = await CoingeckoToken.updateMany(
      { id: { $nin: Array.from(fetchedIds) } },
      {
        $set: {
          current_price: null,
          market_cap: null,
    ...
    console.log(`Deactivated ${result.modifiedCount} tokens.`);
  } catch (err) {
